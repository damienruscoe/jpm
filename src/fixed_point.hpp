...
template <int Exponent, typename Value = uint32_t>
class FixedPoint {
public:
...
template <int Exponent, typename Value>
FixedPoint<Exponent, Value> FixedPoint<Exponent, Value>::operator-(const FixedPoint<Exponent, Value>& rhs) const
{
    return v - rhs.v;
}

template <int Exponent, typename Value>
FixedPoint<Exponent, Value>& FixedPoint<Exponent, Value>::operator-=(const FixedPoint<Exponent, Value>& rhs)
{
    v -= rhs.v;
    return *this;
}
    ...
    const FixedPoint<Exponent, Value>& fixed_point)
{
    if constexpr (Exponent == 0) {
        ...
        os << whole << "." << std::setfill('0') << std::setw(Exponent) << fractional;
